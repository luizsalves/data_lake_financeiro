...
def get_s3_resource():
    ...
    region_name = os.getenv("AWS_DEFAULT_REGION") or os.getenv("AWS_REGION") or os.getenv("MINIO_REGION") or "us-east-1"
    session = boto3.session.Session()
    kwargs = {"service_name": "s3", "region_name": region_name}
    if endpoint_url:
    ...
    return session.resource(**kwargs)
