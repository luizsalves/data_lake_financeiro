...
class AwsGlueJobHook(AwsBaseHook):
    ...
    def initialize_job(self, script_arguments: Optional[dict] = None) -> Dict[str, str]:
        ...
        try:
            ...
            if self.num_of_dpus < 2:
                job_run = glue_client.start_job_run(JobName=job_name, Arguments=script_arguments, MaxCapacity=self.num_of_dpus)
            else:    
                job_run = glue_client.start_job_run(JobName=job_name, Arguments=script_arguments, NumberOfWorkers=self.num_of_dpus, WorkerType='G.1X')
            return job_run
        except Exception as general_error:
    ...
    def job_completion(self, job_name: str, run_id: str) -> Dict[str, str]:
        ...
        while True:
            job_run = self.get_job_state(job_name, run_id)
            ...
            if job_run_state in failed_states:
                ...
                self.log.info("ErrorMessage: {}".format(job_run['ErrorMessage']))
                raise AirflowException(job_error_message)
            else:
                self.log.info(
                    "Polling for AWS Glue Job %s current run state with status %s, run_id %s", job_name, job_run_state, run_id
                )
