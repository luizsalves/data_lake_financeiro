...
INVALID_SPARK_COLUMN_CHARS = re.compile(r'[ ,;{}()\n\t=]+')
AUTO_CATALOG_LAYERS = {"integration", "business"}
...
class ConfProcess:
                        ...
                        "properties": {
                            ...
                            "Output": {
                                ...
                                "properties": {
                                    ...
                                    "PersistExecutionPartitionColumn": {
                                        "type": "boolean",
                                        "description": "When true with Replace/ExecutionDatetime, writes into the execution_ts directory without partitionBy so the execution column is also persisted in the file schema"
                                    },
                                    "OutputFormat": {
                                        "type": "string",
                                        "description": "Physical output format. Supported values in local mode: parquet, iceberg"
                                    },
                                    "CatalogMode": {
                                        "type": "string",
                                        "description": "Catalog strategy for the output. Supported values: hive, iceberg"
                                    },
                                    "DisableCatalog": {
                                        "type": "boolean",
                                        "description": "If true skip automatic metadata catalog synchronization for this output"
                                    },
                                    "IcebergCatalog": {
                                        "type": "string",
                                        "description": "Spark catalog name to use when OutputFormat is iceberg. Default: spark_catalog"
                                    },
    ...
    def __write_output_spark (self,spark,df_input):
        ...
        if self.__job["Output"]["MergeType"] in ["Replace", "Replace/Partition", "Replace/ExecutionDatetime"]:
            ...
            table_format = self.__get_output_format()
            table_mode = 'overwrite'
            persist_execution_partition_column = (
                self.__job["Output"].get("PersistExecutionPartitionColumn", False)
                and self.__job["Output"]["MergeType"] == "Replace/ExecutionDatetime"
            )
            if self.__job["Output"]["MergeType"] == "Replace":
                ...
                if persist_execution_partition_column:
                    spark.conf.set("spark.sql.sources.partitionOverwriteMode","static")
                    spark_options["path"] = '{}/{}={}'.format(
                        self.__job["Output"]["OutputPath"],
                        self.__partition_label,
                        self.__execution_datetime
                    )
                else:
                    spark.conf.set("spark.sql.sources.partitionOverwriteMode","dynamic")
                if not persist_execution_partition_column and self.__partition_label not in partition_keys:
                    partition_keys.append(self.__partition_label )
            if self.__is_iceberg_output():
                self.__sync_iceberg_table(spark, df_input, partition_keys)
            else:
                ...
                write_stm.save()
                self.__sync_catalog_table(spark, df_input, partition_keys)
    ...
    def __get_load_path(self, job_input):
        ...
        if job_input.get('ReadFormat') == 'iceberg' and not str(execution_path).startswith(('s3://', 's3a://', '/')):
    ...
    def __read_from_database(self, read_stm, job_input):
        ...
        if "GlueConnection" in job_input:
            gc = GlueJDBCConection(read_stm, job_input, self.__region)
            df = gc.read_from_odbc()
        else:
            if job_input['ReadFormat'] != 'jdbc':
                raise ConfProcessError(
                    "Direct database connection without GlueConnection is supported only for jdbc"
                )
            required_keys = ("url", "user", "password", "driver")
            missing_keys = [key for key in required_keys if key not in job_input or not job_input[key]]
            if missing_keys:
                raise ConfProcessError(
                    "Direct jdbc input requires keys: {}".format(", ".join(missing_keys))
                )
            ...
            if 'dbtable' not in job_input and 'query' not in job_input:
                raise ConfProcessError("'dbtable' or 'query' is a required property")

            read_stm = read_stm.read.format("jdbc") \
                .option("url", job_input["url"]) \
                .option("user", job_input["user"]) \
                .option("password", job_input["password"]) \
                .option("driver", job_input["driver"])

            if 'dbtable' in job_input:
    ...
    def __sanitize_output_columns(self, df_input):
        sanitized_names = []
        changed_columns = []

        for index, column_name in enumerate(df_input.columns):
            sanitized_name = INVALID_SPARK_COLUMN_CHARS.sub("_", column_name).strip("_")
            sanitized_name = re.sub(r"_+", "_", sanitized_name)
            if not sanitized_name:
                sanitized_name = f"column_{index}"

            if sanitized_name in sanitized_names:
                suffix = 1
                candidate_name = f"{sanitized_name}_{suffix}"
                while candidate_name in sanitized_names:
                    suffix += 1
                    candidate_name = f"{sanitized_name}_{suffix}"
            ...
            if sanitized_name != column_name:
                changed_columns.append((column_name, sanitized_name))

        if changed_columns:
            _logger.warning("Sanitized output columns with invalid Spark characters: %s", changed_columns)
            df_input = df_input.toDF(*sanitized_names)

        return df_input

    def __get_output_layer(self):
        output = self.__job.get("Output", {})
        output_database = output.get("OutputDatabase", "")
        for layer in AUTO_CATALOG_LAYERS:
            if output_database.endswith(f"_{layer}"):
                return layer

        output_path = output.get("OutputPath", "")
        for layer in AUTO_CATALOG_LAYERS:
            if f"/{layer}/" in output_path or output_path.startswith(f"s3a://{layer}/"):
                return layer

        return None

    def __should_catalog_output(self):
        output = self.__job.get("Output", {})
        return (
            not bool(output.get("DisableCatalog", False))
            and self.__get_output_layer() in AUTO_CATALOG_LAYERS
            and bool(output.get("OutputDatabase"))
            and bool(output.get("OutputTable"))
            and bool(output.get("OutputPath"))
            and not self.__is_iceberg_output()
        )

    def __get_output_format(self):
        output = self.__job.get("Output", {})
        explicit_format = str(output.get("OutputFormat", "")).strip().lower()
        if explicit_format:
            return explicit_format
        if self.__get_output_layer() in AUTO_CATALOG_LAYERS and self.__get_catalog_mode() == "iceberg":
            return "iceberg"
        return "parquet"

    def __get_catalog_mode(self):
        output = self.__job.get("Output", {})
        explicit_mode = str(output.get("CatalogMode", "")).strip().lower()
        if explicit_mode:
            return explicit_mode
        return os.getenv("SPARK_CATALOG_MODE", "hive").strip().lower() or "hive"

    def __is_iceberg_output(self):
        return self.__get_output_format() == "iceberg"

    @staticmethod
    def __quote_identifier(identifier):
        return "`{}`".format(str(identifier).replace("`", "``"))

    def __qualified_output_table(self):
        output = self.__job["Output"]
        table_parts = []
        if self.__is_iceberg_output():
            catalog_name = (
                str(output.get("IcebergCatalog", os.getenv("SPARK_ICEBERG_CATALOG_NAME", "spark_catalog"))).strip()
                or "spark_catalog"
            )
            if catalog_name != "spark_catalog":
                table_parts.append(self.__quote_identifier(catalog_name))
        table_parts.extend(
            [
                self.__quote_identifier(output["OutputDatabase"]),
                self.__quote_identifier(output["OutputTable"]),
            ]
        )
        return ".".join(table_parts)

    def __catalog_table_exists(self, spark, database_name, table_name):
        if hasattr(spark.catalog, "tableExists"):
            return spark.catalog.tableExists("{}.{}".format(database_name, table_name))

        result = spark.sql(
            "SHOW TABLES IN {} LIKE '{}'".format(
                self.__quote_identifier(database_name),
                str(table_name).replace("\\", "\\\\").replace("'", "\\'"),
            )
        )
        return result.limit(1).count() > 0

    def __sync_catalog_table(self, spark, df_input, partition_keys):
        if not self.__should_catalog_output():
            return

        output = self.__job["Output"]
        database_name = output["OutputDatabase"]
        table_name = output["OutputTable"]
        output_path = output["OutputPath"]
        qualified_table = self.__qualified_output_table()

        spark.sql("CREATE DATABASE IF NOT EXISTS {}".format(self.__quote_identifier(database_name)))

        partition_key_set = {key.lower() for key in partition_keys}

        if not self.__catalog_table_exists(spark, database_name, table_name):
            column_sql = []
            partition_sql = []
            for field in df_input.schema.fields:
                definition = "{} {}".format(
                    self.__quote_identifier(field.name),
                    field.dataType.simpleString(),
                )
                if field.name.lower() in partition_key_set:
                    partition_sql.append(definition)
                else:
                    column_sql.append(definition)

            create_table_sql = "CREATE EXTERNAL TABLE IF NOT EXISTS {table} ({columns})".format(
                table=qualified_table,
                columns=", ".join(column_sql),
            )
            if partition_sql:
                create_table_sql += " PARTITIONED BY ({})".format(", ".join(partition_sql))
            create_table_sql += " STORED AS PARQUET"
            create_table_sql += " LOCATION '{}'".format(output_path.replace("'", "\\'"))
            create_table_sql += " TBLPROPERTIES ('classification'='parquet')"
            spark.sql(create_table_sql)
        else:
            existing_columns = {
                field.name.lower(): field.dataType.simpleString()
                for field in spark.table(qualified_table).schema.fields
            }
            missing_columns = []
            ...
            for field in df_input.schema.fields:
                ...
                if field_name not in existing_columns:
                    if field_name in partition_key_set:
                        missing_partition_columns.append(field.name)
                        continue
            ...
            if missing_columns:
                spark.sql(
                    "ALTER TABLE {} ADD COLUMNS ({})".format(
                        qualified_table,
            ...
            if missing_partition_columns:
                _logger.warning(
                    "Catalog table [%s] is missing partition columns [%s]; automatic partition spec changes are not supported",
                    qualified_table,
                    ", ".join(missing_partition_columns),
                )

        spark.sql(
            "ALTER TABLE {} SET TBLPROPERTIES ('classification'='parquet')".format(
                qualified_table
            )
        )

        if partition_keys:
            spark.sql("MSCK REPAIR TABLE {}".format(qualified_table))
        spark.catalog.refreshTable(qualified_table)

    def __sync_iceberg_table(self, spark, df_input, partition_keys):
        output = self.__job["Output"]
        output_path = output.get("OutputPath", "")
        merge_type = output["MergeType"]
        qualified_table = self.__qualified_output_table()
        temp_view_name = "__tmp_{}_{}".format(
            self.get_job_name().lower(),
            str(random()).replace(".", ""),
        )
        ...
        if catalog_name != "spark_catalog":
            database_parts.append(self.__quote_identifier(catalog_name))
        database_parts.append(self.__quote_identifier(output["OutputDatabase"]))
        spark.sql("CREATE DATABASE IF NOT EXISTS {}".format(".".join(database_parts)))

        column_sql = ", ".join(
            "{} {}".format(self.__quote_identifier(field.name), field.dataType.simpleString())
            for field in df_input.schema.fields
        )
        create_table_sql = "CREATE TABLE IF NOT EXISTS {table} ({columns}) USING iceberg".format(
            table=qualified_table,
            columns=column_sql,
        )
        if partition_keys:
            create_table_sql += " PARTITIONED BY ({})".format(
                ", ".join(self.__quote_identifier(key) for key in partition_keys)
            )
        if output_path:
            create_table_sql += " LOCATION '{}'".format(output_path.replace("'", "\\'"))
        spark.sql(create_table_sql)

        df_input.createOrReplaceTempView(temp_view_name)
        try:
            if merge_type == "Replace":
                replace_sql = "REPLACE TABLE {table} USING iceberg".format(table=qualified_table)
                if partition_keys:
                    replace_sql += " PARTITIONED BY ({})".format(
                        ", ".join(self.__quote_identifier(key) for key in partition_keys)
                    )
                if output_path:
                    replace_sql += " LOCATION '{}'".format(output_path.replace("'", "\\'"))
                replace_sql += " AS SELECT * FROM {}".format(self.__quote_identifier(temp_view_name))
                spark.sql(replace_sql)
            else:
                spark.conf.set("spark.sql.sources.partitionOverwriteMode", "dynamic")
                spark.sql(
                    "INSERT OVERWRITE {} SELECT * FROM {}".format(
                        qualified_table,
                        self.__quote_identifier(temp_view_name),
                    )
                )
            spark.catalog.refreshTable(qualified_table)
        finally:
    ...
    def process_spark(self,spark):
        ...
        df_udf = self.__sanitize_output_columns(df_udf)
